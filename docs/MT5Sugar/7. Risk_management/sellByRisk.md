# 🔴 SELL with Automatic Risk-Based Volume
...
2. Gets current Bid price
3. Calculates SL price: `Bid + (stopLossPoints × point)`
4. Calculates TP price: `Bid - (takeProfitPoints × point)` (if > 0)
5. Places SELL market order
...
* **SL placement:** `Bid + (stopLossPoints × point)`
* **TP placement:** `Bid - (takeProfitPoints × point)` (if > 0)
...
* **BUY version:** [`buyByRisk()`](./buyByRisk.md)
* **Volume calculator:** [`calculateVolume()`](./calculateVolume.md)
* **Manual volume:** [`sellMarket()`](../2.%20Market_orders/sellMarket.md)
